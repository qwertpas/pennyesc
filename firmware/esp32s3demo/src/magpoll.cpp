#include <Arduino.h>
#include "esc_address.h"

#define BLDC_RX 13
#define BLDC_TX 12
#define BLDC_GND 11
#define BLDC_3V3_PULLUP 9
#define BLDC_BAUD 921600

#define START_BYTE 0xAA
#define CMD_POLL 0x03
#define RESPONSE_LEN 17

static const uint8_t crc8_table[256] PROGMEM = {
    
};

struct PollResponse {
    bool valid;
    int16_t x;
    int16_t y;
    int16_t z;
};

struct Stats {
    uint32_t sent = 0;
    uint32_t received = 0;
    uint32_t crcErrors = 0;
    uint32_t timeouts = 0;
};

static Stats stats;
static uint8_t selectedEsc = ESC1;
static String inputLine;

static uint8_t crc8(const uint8_t *data, size_t len) {
    uint8_t crc = 0x00;
    while (len--) {
        crc = pgm_read_byte(&crc8_table[crc ^ *data++]);
    }
    return crc;
}

static PollResponse pollEsc(uint8_t address) {
    PollResponse resp = {false, 0, 0, 0};
    uint8_t txBuf[3];
    txBuf[0] = START_BYTE;
    txBuf[1] = (address << 4) | CMD_POLL;
    txBuf[2] = crc8(txBuf, 2);
    Serial1.write(txBuf, sizeof(txBuf));
    stats.sent++;

    uint8_t rxBuf[RESPONSE_LEN];
    uint8_t rxIdx = 0;
    uint32_t timeout = millis() + 20;

    while (rxIdx < RESPONSE_LEN && millis() < timeout) {
        if (!Serial1.available()) {
            continue;
        }

        uint8_t b = Serial1.read();
        if (b == START_BYTE) {
            rxIdx = 0;
            rxBuf[rxIdx++] = b;
            continue;
        }
        if (rxIdx == 0) {
            continue;
        }
        rxBuf[rxIdx++] = b;
    }

    if (rxIdx < RESPONSE_LEN) {
        stats.timeouts++;
        return resp;
    }

    if (crc8(rxBuf, RESPONSE_LEN - 1) != rxBuf[RESPONSE_LEN - 1]) {
        stats.crcErrors++;
        return resp;
    }

    resp.valid = ((rxBuf[1] >> 4) & 0x0F) == address;
    resp.x = (int16_t)(rxBuf[2] | (rxBuf[3] << 8));
    resp.y = (int16_t)(rxBuf[4] | (rxBuf[5] << 8));
    resp.z = (int16_t)(rxBuf[6] | (rxBuf[7] << 8));
    if (resp.valid) {
        stats.received++;
    }
    return resp;
}

static void printHelp() {
    Serial.println("# magpoll");
    Serial.println("# commands: e<esc>, ?, h");
}

static void handleLine(String line) {
    line.trim();
    if (!line.length()) {
        return;
    }

    char cmd = line.charAt(0);
    if (cmd == 'e' || cmd == 'E') {
        int esc = line.substring(1).toInt();
        if (esc >= 0 && esc < ESC_COUNT) {
            selectedEsc = (uint8_t)esc;
            Serial.printf("# esc=%u\n", selectedEsc);
        } else {
            Serial.println("# invalid esc");
        }
        return;
    }

    if (cmd == '?') {
        Serial.printf(
            "# esc=%u sent=%lu recv=%lu crc=%lu timeout=%lu\n",
            selectedEsc,
            stats.sent,
            stats.received,
            stats.crcErrors,
            stats.timeouts
        );
        return;
    }

    if (cmd == 'h' || cmd == 'H') {
        printHelp();
        return;
    }

    Serial.println("# unknown");
}

void setup() {
    pinMode(BLDC_GND, OUTPUT);
    digitalWrite(BLDC_GND, LOW);

    pinMode(BLDC_3V3_PULLUP, OUTPUT);
    digitalWrite(BLDC_3V3_PULLUP, HIGH);

    Serial.begin(115200);
    Serial1.begin(BLDC_BAUD, SERIAL_8N1, BLDC_RX, BLDC_TX);
    delay(100);
    printHelp();
    Serial.printf("# esc=%u\n", selectedEsc);
}

void loop() {
    static uint32_t lastPollUs = 0;
    uint32_t now = micros();
    if ((uint32_t)(now - lastPollUs) >= 2000) {
        lastPollUs = now;
        PollResponse resp = pollEsc(selectedEsc);
        if (resp.valid) {
            Serial.printf("%u,%d,%d,%d\n", selectedEsc, resp.x, resp.y, resp.z);
        }
    }

    while (Serial.available()) {
        char ch = (char)Serial.read();
        if (ch == '\n' || ch == '\r') {
            if (inputLine.length()) {
                handleLine(inputLine);
                inputLine = "";
            }
        } else if (inputLine.length() < 32) {
            inputLine += ch;
        }
    }
}
