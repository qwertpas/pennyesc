...
RESULTS_DIR = Path(__file__).resolve().parent / "penny-gui-sessions"
        ...
        try:
            with EspBridge(self.port) as bridge:
                ...
                try:
                    ...
                    self.log.emit("brake motor")
                    status = client.brake()
                    if status.result != RESULT_OK:
                        raise CalibrationError(f"BRAKE failed with result {status.result}")
...
class Window:
    def __init__(self, port: str, address: int, chunk_size: int, auto: bool, session_path: Path) -> None:
        ...
        self.win.setWindowTitle("penny-gui")
    ...
    def run_terminal_command(self, text: str) -> None:
        ...
        cmd = parts[0].lower()
        if cmd == "help":
            self.append_log("commands: status | control | kp/kd/kv <gain> | kf <duty> | clip <duty> | pos <rad> | vel <rad/s> | rpm <rpm> | mode pos|vel|duty [duty] | zero | brake | advance <deg>")
